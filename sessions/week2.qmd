...
::: {.incremental}
We can try different sampling approaches
  - random sampling 
  - systematic sampling 
:::
...
::: {.notes}
Example of cognitive bias - confirmation bias, whereby I favour information that agrees with my existing beliefs. 
:::
...
::: {.notes}
If we're more aware of our biases it can be easier to try and actively avoid them. 
:::
...
Tools also used to help the sentencing of criminals in courts of law - observe the hungry judges effect, where they sentnece differently based on how hungry they are - tend to be harsher just before lunch and more lenient after lunch. 
:::
...
WEIRD = western, educated, industrialised, rich, democratic. Often studied in healthcare, and results assumed to apply to global population - but in reality actually not representative - at best representative of a small fraction of global pop.  
:::
...
The most basic statistical information about the dataset. 

. . .

- Sample size (n) 
- Mean, median, mode (depending on data type)
- Standard deviation 
- Range 
...
These four datasets all have the same mean and variance of both $x$ and $y$. 
...
- Characteristic size, all data points are close to the mean -> single peak
- Data is equally likely to be larger or smaller than average -> symmetric
- There is less data further away from the mean -> smooth tails on both sides
...
When modelling, we need to account for the fact that we don't have all the data. 
...
::: {.notes}
Generally want the probability that $x>p$ or $x<=p$ -> this is the area under the curve.
:::
...
But not all ---  might not be continuous, might not have a characteristic size, or might not be symmetric. 
...
::: {.notes}
Thinking back to data types as we discussed last week. 

Suppose I have someone who is 170 cm tall, and someone who is 171 cm tall, I can find someone in between whose 170.5cm tall. 
...
<div style="text-align:center;">
  <img src="L2_images/coin_flip.jpg" alt="A hand flipping a coin." style="max-width:60%">
</div>
...
::: {.incremental}
- Imagine a situation where certain events can occur in an **independent** fashion. 
...
- Between 1883 and 1893 there were an average of 2 deaths from horse kicks a year. 
  - **$\lambda=2$**
- What's the probability of seeing 10 deaths from horse kicks in the year 1894? 
::: 
...
::: {.notes}
Nothing to do with the Bartlett school. Also, he's a bit dramatic
:::
...
::: {.notes}
Ancient legend about the wise sage who invented chess. 
...
::: {.incremental}
- which is more rice than the world's total rice production...  
:::
...
:::{.notes}
Its a mathematical constant simialr to pi - it's just a fixed number which we have a name for. 
...
The exponential function allows us to answer questions like: 

::: {.incremental}
- "how many grains of rice at t=10?"
- or “when will I have enough rice to feed my entire town?” 
:::

Answering the second question is easier said than done – the best way is to invert the equation.
